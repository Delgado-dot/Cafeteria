...
async function renderCafePill(el) {
  try {
    ...
    el.innerHTML = `<span class="badge ${isOpen ? 'badge-success' : 'badge-danger'}"><span class="ico"><i class="bx ${isOpen ? 'bx-check-circle' : 'bx-x-circle'}"></i></span> ${isOpen ? 'ABIERTA' : 'CERRADA'}</span>`;
  } catch (e) {
    el.innerHTML = `<span class="badge badge-success"><span class="ico"><i class="bx bx-check-circle"></i></span> ABIERTA</span>`;
  }
}
...
async function barConfigTabs(el, initialTab) {
  ...
  if (!configRes.ok) {
    el.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudo cargar la configuración');
    return;
  }
...
async function barSuppliers(el) {
  ...
  const renderSuppliersPage = async () => {
    ...
    if (!res.ok) {
      wrap.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudieron cargar los proveedores');
      return;
    }
...
async function barDashboard(el) {
  ...
  el.innerHTML = `
    ...
    <div class="status-banners-wrap">
      ${outStock.length ? `<div class="status-banner danger"><span class="ico"><i class="bx bx-x-circle"></i></span><div><b>Productos agotados:</b> ${outStock.map((p) => p.name).join(', ')}</div></div>` : ''}
    ...
    <div class="card" style="margin-bottom:20px;${hasPriority ? 'border-left:4px solid var(--primary)' : ''}">
      <div class="card-header">
        <div><div class="card-title"><i class="bx bx-zap" style="margin-right:4px"></i>Pedidos prioritarios</div><div class="card-sub">Atiende primero los pedidos urgentes, de prioridad o delivery</div></div>
...
function priorityMiniCard(o) {
  const priTag = o.priority === 'urgent'
    ? `<span class="priority-tag urgent"><i class="bx bx-zap"></i> Urgente</span>`
...
async function barOrders(el) {
  ...
  if (!ordersRes.ok) {
    el.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudieron cargar los pedidos');
    return;
  }
  ...
  const render = () => {
    ...
    if (!list.length) { queueArea.innerHTML = emptyState('<i class="bx bx-receipt"></i>', 'Sin pedidos', 'No hay pedidos en esta sección.'); return; }
...
function queueOrderCard(o, tab) {
  ...
  let extraCls = '';
  let priTag = '';
  if (o.priority === 'urgent') { extraCls += ' pri-urgent'; priTag = `<span class="priority-tag urgent"><i class="bx bx-zap"></i> Urgente</span>`; }
...
async function barProducts(el) {
  ...
  if (!productsRes.ok) {
    el.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudieron cargar los productos');
    return;
  }
...
async function barStock(el) {
  ...
  if (!productsRes.ok) { el.innerHTML = emptyState('<i class="bx bx-error"></i>','Error','No se pudo cargar stock'); return; }
...
async function barPayments(el) {
  ...
  if (!paymentsRes.ok) {
    el.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudieron cargar los pagos');
    return;
  }
...
async function barSalesDashboard(el) {
  ...
  if (!ordersRes.ok) { el.innerHTML = emptyState('<i class="bx bx-error"></i>','Error','No se pudo cargar ventas'); return; }
...
const days = [];
  ...
  if (validSales.length >= 3 && days.length === 7) {
    ...
    if (avgWeek > 0 && salesToday > avgWeek * 1.1) {
      microMsg = `Hoy vas mejor que el promedio de la semana <i class="bx bx-trending-up"></i>`;
...
async function barDelivery(el) {
  ...
  if (!ordersRes.ok) {
    el.innerHTML = emptyState('<i class="bx bx-error"></i>', 'Error', 'No se pudieron cargar los pedidos de delivery');
    return;
  }
  ...
  el.innerHTML = `
    ...
    ${cfg.deliveryEnabled ? '' : '<div class="status-banner warning" style="margin-top:16px" id="dlWarning"><span class="ico"><i class="bx bx-error"></i></span><div>Delivery interno deshabilitado. Habilítalo en Configuración.</div></div>'}
  `;
  ...
  const renderDelivery = () => {
    ...
    if (!list.length) {
      area.innerHTML = `<div class="empty-state" style="padding:24px"><div class="es-ico"><i class="bx bx-box"></i></div><h3>Sin pedidos ${dtab}</h3><p class="tiny muted">No hay deliveries en este estado por ahora.</p></div>`;
      return;
    }
