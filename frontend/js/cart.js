...
const CartService = {
  get items() { return Store.load('int_cart', []); },
  set items(val) { Store.save('int_cart', val); },

  save(items) { this.items = items; },
  ...
  addItem(product, qty, addons, note) {
    ...
    if (ex) {
      ...
      items.push({
        ...
        qty,
        name,
        price: product.price + (addons?.reduce((s, a) => s + a.price, 0) || 0),
        basePrice: product.price,
        addons: addons || [],
        note: note || '',
        emoji: clientProductIcon(product),
        prepMin: product.prepMin,
      });
    }
    this.save(items);
    return { ok: true };
...
const CafeService = {
  get info() {
    ...
    if (pct >= 100) {
      state = 'CAPACIDAD LLENA';
      stateCls = 'danger';
    } else if (pct >= 70) {
      state = 'ALTA DEMANDA';
      stateCls = 'warning';
      warnMsg = 'Alta demanda. Tu pedido podría tardar más de lo habitual.';
    }
...
const OrderService = {
  ...
  createOrder(user, items, total, delivery, payMethod, prepMin) {
    const now = new Date();
    ...
    return {
      ...
      deliveryInfo: delivery === 'delivery' ? window._deliveryInfo : null, // Temporary fix, should use CheckoutState
      payment: payMethod,
...
const CheckoutState = {
  ...
  setVoucher(attached) {
    this.voucherAttached = attached;
  },

  reset() {
    this.deliveryInfo = null;
    this.payMethod = 'deuna';
    this.voucherAttached = false;
  },
};

/* ------------------------------------------------------------
   CART FACADE (Public API)
   ------------------------------------------------------------ */

const Cart = {
  get items() { return CartService.items; },

  count() { return CartService.count(); },
  total() { return CartService.total(); },
  ...
  add(product, qty, addons, note) {
    ...
    if (newQty > product.stock) {
      return { ok: false, msg: 'No puedes agregar más de ' + product.stock + ' unidades (stock).' };
    }

    const result = CartService.addItem(product, qty, addons, note);
    if (result.ok) refreshCartBadge();
    return result;
  },
  ...
  setQty(productId, qty) {
    ...
    if (qty > product.stock) {
      toast('No puedes superar el stock disponible (' + product.stock + ').', 'warning');
      return;
    }
  ...
  canOrder() {
    return CafeService.canPlaceOrder();
  },

  getCapacity() {
    return CafeService.info;
  },

  checkout(user) {
    const cap = CafeService.info;
    if (cap.stateCls === 'danger') {
      return { ok: false, msg: 'La capacidad está completa. No se puede confirmar el pedido.' };
    }

    if (CheckoutState.payMethod === 'delivery' && !CheckoutState.deliveryInfo) {
       // This logic was in confirmOrder, but the method was 'delivery'
    }

    // Validation logic from confirmOrder
    const delivery = CheckoutState.deliveryInfo ? 'delivery' : 'pickup';
    if (delivery === 'delivery' && !CheckoutState.deliveryInfo) {
       return { ok: false, msg: 'Selecciona el piso y el aula para el delivery interno.' };
    }
    if (CheckoutState.payMethod === 'transferencia' && !CheckoutState.voucherAttached) {
       return { ok: false, msg: 'Carga el comprobante de transferencia (simulado).' };
    }
...
/* ------------------------------------------------------------
   GLOBAL HELPERS (Maintaining backward compatibility)
   ------------------------------------------------------------ */
...
window.capacityInfo = capacityInfo;

function renderCapacityCard(container) {
  const info = Cart.getCapacity();
  let cls = 'bar-fill';
...
function capacityAllows() {
  return Cart.getCapacity().pct < 100;
}
window.capacityAllows = capacityAllows;

function cafeStatus() {
  return CafeService.getStatus();
}
window.cafeStatus = cafeStatus;

function canPlaceOrder() {
  return Cart.canOrder();
}
...
function renderCheckout(el) {
  ...
  const cfg = Store.config;
  const deliveryOn = cfg.deliveryEnabled && Cart.canOrder();
  const payOptions = [
  ...
  deliveryWrap.querySelectorAll('[data-d]').forEach((el) => {
    el.onclick = () => {
      ...
      if (el.dataset.d === 'delivery') renderAulaSelector();
        ...
        CheckoutState.setDelivery(null);
      }
    };
  });
  ...
  function renderAulaSelector() {
    ...
    function updateConfirm() {
      ...
      if (state.aula) {
        ...
        CheckoutState.setDelivery({ piso: state.piso, aula: state.aula });
      } else {
  ...
  payWrap.querySelectorAll('[data-p]').forEach((el) => {
    el.onclick = () => {
      ...
      el.classList.add('active');
      CheckoutState.setPayment(el.dataset.p);
      renderPayDetail(el.dataset.p);
    };
  });
  CheckoutState.setPayment('deuna');
  ...
  function renderPayDetail(method) {
    ...
    if (method === 'deuna') {
      detail.innerHTML = \`
        <div class="alert info" style="margin-bottom:16px"><span class="a-ico">\${clientIcon('mobile')}</span><div><div class="a-title">Pago con DEUNA.</div>Escanea el código QR para pagar \${money(Cart.total())}. Se validará en línea y se mostrará en el estado del pedido.</div></div>
      ...
      detail.innerHTML = \`
        <div class="alert info" style="margin-bottom:16px"><span class="a-ico">\${clientIcon('transfer')}</span><div><div class="a-title">Transferencia.</div>Realiza una transferencia por \${money(Cart.total())} y adjunta el comprobante (simulado).</div></div>
...
function confirmOrder() {
  ...
  const result = Cart.checkout(user);
