...
// --- Catálogo granular de permisos (funcional, no en blanco) ---
// Cada permiso tiene code único "modulo.accion". Se agrupan por categoría para UI.
...
const Store = {
  ...
  reset() {
    this._cache = {};
    ['int_products', 'int_orders', 'int_users', 'int_config', 'int_audit', 'int_stockHistory', 'int_rolePerms'].forEach((k) => localStorage.removeItem(k));
  },
};

// Helpers de permisos — control real de acceso en frontend
function getRolePerms(role) {
  const all = Store.rolePerms;
  if (all[role]) return all[role];
  // rol personalizado: sin permisos por defecto
  return {};
}
...
function can(code) {
  ...
  if (!u) return false;
  // admindev con permissions.edit puede todo si quiere, pero respetamos granular
...
function setRolePerm(role, code, enabled) {
  ...
  Store.rolePerms = all;
  // intentar sincronizar con backend si hay API disponible (no bloqueante)
  try {
    if (typeof fetch !== 'undefined') {
      const token = (()=>{ try{ const s=JSON.parse(localStorage.getItem('int_session')); return s?.token||'';}catch(e){return ''}})();
      // no enviar si no hay backend corriendo; silencioso
